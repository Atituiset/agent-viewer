...
// agent-session-format 的 opencode 查询需要 version/summary_* 列与 project 表。
function createSchema(db: Database.Database): void {
  db.exec(`CREATE TABLE project(id TEXT, worktree TEXT);
           CREATE TABLE session(id TEXT, project_id TEXT, title TEXT, directory TEXT, version TEXT,
                                summary_additions INTEGER, summary_deletions INTEGER, summary_files INTEGER,
                                model TEXT, cost INTEGER, tokens_input INTEGER, tokens_output INTEGER,
                                tokens_reasoning INTEGER, time_created INTEGER, time_updated INTEGER, agent TEXT);
...
function makeDb(): Buffer {
  ...
  createSchema(db);
  db.prepare("INSERT INTO project VALUES (?,?)").run("p", "/d");
  db.prepare("INSERT INTO session VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run("s1", "p", "T", "/d", "1.0", 0, 0, 0, '"deepseek-v3"', 0, 10, 20, 0, 1735689600000, 0, null);
...
function writeHomeDb(buf: Buffer): string {
...
describe("opencode parser", () => {
  ...
  it("skips corrupt rows instead of failing the whole session", async () => {
    ...
    createSchema(db);
    db.prepare("INSERT INTO project VALUES (?,?)").run("p", "/d");
    db.prepare("INSERT INTO session VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run("s1", "p", "T", "/d", null, 0, 0, 0, null, 0, 0, 0, 0, "not-a-number", 0, null);
    ...
    const src = new LocalFileSource(writeHomeDb(buf));
